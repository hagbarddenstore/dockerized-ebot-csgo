...
class Match implements Taskable {
    ...
    public function __construct($match_id, $server_ip, $rcon) {
        ...
        try {
            ...
            $external_ip = \eBot\Config\Config::getInstance()->getBot_publicIp();
            $external_port = \eBot\Config\Config::getInstance()->getBot_publicPort();
            $this->rcon->send("log on; mp_logdetail 3; logaddress_del " . $external_ip . ":" . $external_port . ";logaddress_add " . $external_ip . ":" . $external_port);
            $this->rcon->send("sv_rcon_whitelist_address \"" . $external_ip . "\"");
...
//            $this->rcon->send("tv_stoprecord");
            if (\eBot\Config\Config::getInstance()->getDemoDownload()) {
                $external_ip = \eBot\Config\Config::getInstance()->getBot_publicIp();
                $external_port = \eBot\Config\Config::getInstance()->getBot_publicPort();
                $this->rcon->send('tv_stoprecord; ' . 'csay_tv_demo_push "' . $this->currentRecordName . '.dem" "http://' . $external_ip . ':' . $external_port . '/upload"');
            } else {
                $this->rcon->send("tv_stoprecord");
            }
